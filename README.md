...
### 학생 연락 (무료·수동, 문자 기반)
- 사물함 상세 패널과 **‘마감·임박 사물함’** 목록에서 **문자** 버튼을 제공합니다. (전화 걸기 버튼은 사용 안 함)
- 버튼을 누르면 **직원 기기의 문자앱**이 **안내 문구가 채워진 채로** 열립니다.
- PC에서 문자앱이 없을 때를 위해 **‘복사’** 버튼도 제공합니다.
- 한 명씩 보내는 방식(가입·비용 없음). 자동 알림톡/SMS가 필요하면 추후 발송대행사 +
  Supabase Edge Function 으로 확장 가능합니다.
