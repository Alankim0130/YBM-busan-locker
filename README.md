...
혹시 모를 상황(실수 삭제·DB 장애 등) 대비로, Supabase 데이터를 구글 시트에 백업합니다.
모든 변화를 **월별 누적 기록(append-only)**으로 영구 보존하므로, 데이터가 사라져도 그 순서대로 따라가며 복원할 수 있습니다.
...
### 결과 (탭 구성)
- **`YYYY-MM` 월별 기록 탭**: 변화가 생길 때마다 한 줄씩 시간순으로 쌓이는 **누적(append-only)** 기록.
  등록/입금·반납신청·**반납완료(보증금 환급)**·연장·이동·정보수정·종강일 변경이 모두 기록됩니다.
  한 번 쌓인 줄은 다시 지워지지 않고, **최근 12개월**만 유지(더 오래된 월 탭은 자동 삭제)합니다.
- **현황** 탭: 현재 사용중 현황(층·번호·학생·전화·반·종강일·마감일·연장·보증금·결제·상태) + 마지막 백업 시각
- **초기화·고장** 탭: 비밀번호 초기화(1004) 필요 / 고장난 사물함
- **rentals / classes / lockers / requests / notices / settings** 탭: 복구용 원자료 전체(스냅샷)

> 스냅샷 탭은 매번 최신 상태로 덮어쓰고, 월별 탭에는 새 변화만 덧붙입니다(중복은 '키' 열로 방지).
> 예전 버전의 단일 `logs` 탭이 남아 있으면 월별 탭으로 대체되므로 한 번 수동 삭제해도 됩니다.
